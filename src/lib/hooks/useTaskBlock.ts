...
const useTaskBlock = ({ task, isMini }: useTaskBlockParameter) => {
	...
	const onDragStopHandler = (e: DragEvent, d: DraggableData) => {
		...
		if (overlappedTask) {
			...
			if (direction == 'below') {
				...
				aheadTasks = aheadTasks
					.slice(0, overlappedTaskIdx + 1)
					.reverse();
	...
	const onTaskFormSubmit = useCallback((e: SubmitEvent) => {
		e?.preventDefault();
		e?.stopPropagation();
		setShowTaskForm(false);
	}, []);

	const onTaskFormCancel = useCallback((e: SubmitEvent) => {
		e?.preventDefault();
		e?.stopPropagation();
		setShowTaskForm(false);
	}, []);
